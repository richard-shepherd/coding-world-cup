...
namespace BootAndShoot
{
    ...
    class BootAndShoot : CodingWorldCupAPI
    {
        ...
        protected override void processRequest_ConfigureAbilities(dynamic data)
        {
            ...
            //left defender
            playerInfo.add("playerNumber", this.playerNumber_LeftWingDefender);
            playerInfo.add("kickingAbility", 100);
            playerInfo.add("runningAbility", 100);
            playerInfo.add("ballControlAbility", 100);
            playerInfo.add("tacklingAbility", 100);
            playerInfos.Add(playerInfo);

            playerInfo = new JSObject();
            //center defender
            playerInfo.add("playerNumber", this.playerNumber_CentreDefender);
            playerInfo.add("kickingAbility", 50);
            playerInfo.add("runningAbility", 40);
            playerInfo.add("ballControlAbility", 50);
            playerInfo.add("tacklingAbility", 60);
            playerInfos.Add(playerInfo);

            playerInfo = new JSObject();
            //right defender
            playerInfo.add("playerNumber", this.playerNumber_RightWingDefender);
            playerInfo.add("kickingAbility", 50);
            playerInfo.add("runningAbility", 40);
            playerInfo.add("ballControlAbility", 50);
            playerInfo.add("tacklingAbility", 60);
            ...
            playerInfo = new JSObject();
            //left fwd
            playerInfo.add("playerNumber", this.playerNumber_LeftWingForward);
            playerInfo.add("kickingAbility", 50);
            playerInfo.add("runningAbility", 55);
            playerInfo.add("ballControlAbility", 50);
            playerInfo.add("tacklingAbility", 80);
            playerInfos.Add(playerInfo);

            playerInfo = new JSObject();
            //right fwd
            playerInfo.add("playerNumber", this.playerNumber_RightWingForward);
            playerInfo.add("kickingAbility", 100);
            playerInfo.add("runningAbility", 100);
            playerInfo.add("ballControlAbility", 100);
            playerInfo.add("tacklingAbility", 100);
            ...
            playerInfo = new JSObject();
            //goal keeper
            playerInfo.add("playerNumber", this.goalkeeperPlayerNumber);
            playerInfo.add("kickingAbility", 50);
            playerInfo.add("runningAbility", 65);
            playerInfo.add("ballControlAbility", 50);
            playerInfo.add("tacklingAbility", 0);
            playerInfos.Add(playerInfo);
        ...
        private void getAction_Defender_HasBall(List<JSObject> actions, dynamic player)
        {
            ...
            var defenderPosition = new Position(player.dynamicState.position);
            var leftWingForward = this.allTeamPlayers[this.playerNumber_LeftWingForward];
            var leftWingForwardPosition = new Position(leftWingForward.dynamicState.position);
            var rightWingForward = this.allTeamPlayers[this.playerNumber_RightWingForward];
            var rightWingForwardPosition = new Position(rightWingForward.dynamicState.position);

            double distanceBtwnLftFwdAndDefender = defenderPosition.distanceFrom(leftWingForwardPosition);
            double distanceBtwnRightFwdAndDefender = defenderPosition.distanceFrom(rightWingForwardPosition);
            var destination = leftWingForwardPosition;
            if (distanceBtwnLftFwdAndDefender < distanceBtwnRightFwdAndDefender)
            {
                destination = leftWingForwardPosition;
            }
            else
            {
                destination = rightWingForwardPosition;
        ...
        private void getAction_Goalkeeper_HasBall(List<JSObject> actions)
        {
            ...
            var goalkeeper = this.allTeamPlayers[this.goalkeeperPlayerNumber];
            var goalkeeperPosition = new Position(goalkeeper.dynamicState.position);
           
            
            var leftWingDefender = this.allTeamPlayers[this.playerNumber_LeftWingDefender];
            var leftWingDefenderPosition = new Position(leftWingDefender.dynamicState.position);
            var rightWingDefender = this.allTeamPlayers[this.playerNumber_RightWingDefender];
            var rightWingDefenderPosition = new Position(rightWingDefender.dynamicState.position);

            double distanceBtwnLftDefenderAndgoalkeepr = goalkeeperPosition.distanceFrom(leftWingDefenderPosition);
            double distanceBtwnRytDefenderAndgoalkeepr = goalkeeperPosition.distanceFrom(rightWingDefenderPosition);
            var destination = leftWingDefenderPosition;
            if (distanceBtwnLftDefenderAndgoalkeepr < distanceBtwnRytDefenderAndgoalkeepr)
            {
                destination = leftWingDefenderPosition;
            }
            else
            {
                destination = rightWingDefenderPosition;
            ...
            Logger.log(String.Format("Goalkeeper kicks the ball to {0}", destination), Logger.LogLevel.INFO);
        }
